...
class ComPermissionController extends Controller
{
    ...
    public function store(ComPermissionRequest $request)
    {
        ...
        $systems = \App\Models\System::with('metrics')->get();
        $permissionObject = $data['permissionObject'] ?? [];
        foreach ($systems as $system) {
            $sysViewKey = 'SYSTEM_' . $system->id . '_VIEW';
            if (!isset($permissionObject[$sysViewKey]) || !$permissionObject[$sysViewKey]) {
                foreach ($system->metrics as $metric) {
                    $permissionObject['KPI_' . $metric->id . '_VIEW'] = false;
                    $permissionObject['KPI_' . $metric->id . '_CREATE'] = false;
                    $permissionObject['KPI_' . $metric->id . '_EDIT'] = false;
                    $permissionObject['KPI_' . $metric->id . '_DELETE'] = false;
                }
            }
        }
        $data['permissionObject'] = $permissionObject;
